...
public:
	inline				draw_triangles_command	( debug_renderer& renderer, vertices_type const& vertices, indices_type const& indices );
	template < int vertex_count, int index_count >
	inline				draw_triangles_command	( debug_renderer& renderer, vertex_colored const ( &vertices )[ vertex_count ], u16 const ( &indices )[ index_count ] );
	template < template < typename > class container_type >
	inline				draw_triangles_command	( debug_renderer& renderer, container_type < vertex_colored > const& vertices, container_type < u16 > const& indices );
