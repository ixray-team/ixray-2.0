...
void simulator_states::set_rs(u32 a, u32 b)
{
	// Search duplicates
	for (int it = 0; it < int(m_states.size()); ++it)
	{
		state& st	= m_states[it];
		if ((0 == st.type) && (a == st.v1))
		{	// Needed to replace instead of erase.
	...
	sim_st.set_rs(a,b);
	m_states.push_back(sim_st);
}

void simulator_states::set_samp(u32 a, u32 b, u32 c)
{
	// Search duplicates
	for (int it = 0; it < int(m_states.size()); ++it)
	{
		state& st	= m_states[it];
		if ((2 == st.type) && (a == st.v1) && (b == st.v2))
		{
			m_states.erase(m_states.begin()+it);
			break;
		}
	...
	// Register
	state		sim_st;
	sim_st.set_samp	(a,b,c);
	m_states.push_back(sim_st);
}

bool	simulator_states::equal	(simulator_states& other)
{
	if (m_states.size()!=other.m_states.size())
		return false;

	void* mem_ptr_1 = &*m_states.begin();
	void* mem_ptr_2 = &*other.m_states.begin();
	size_t	size = m_states.size()*sizeof(simulator_states::state);

	return 0 == memcmp(mem_ptr_1, mem_ptr_2, size);
}

void	simulator_states::clear	()
{
	m_states.clear();
}
